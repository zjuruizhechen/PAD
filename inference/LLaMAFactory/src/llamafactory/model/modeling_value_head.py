...
class CausalLMOutputWithPast:
    def __init__(self, logits, loss, attention_mask=None):
        # 在初始化时接受 logits 并赋值给类的实例变量
        self.logits = logits
        self.loss = loss
        self.attentions = attention_mask
...
class AutoModelForCausalLMWith2ValueHead(PreTrainedModelWrapper):
    ...
    Class attributes:
        ...
        - **supported_args** (`tuple`) -- A tuple of strings that are used to identify the arguments that are supported
            by the `ValueHead` class. Currently, the supported inference are:
    ...
    def forward(
        ...
        # lm_logits = self.pretrained_model.lm_head(value)
        print(value.shape)
        print(last_hidden_state.shape)
        lm_logits = self.pretrained_model.lm_head(value)
        # base_model_output.hidden_states[-1] = last_hidden_state
        base_model_output.logits = lm_logits
...
class AutoModelForCausalLMWithDPOValueHead(PreTrainedModelWrapper):
    r"""
    An autoregressive model with a value head in addition to the language model head.
    This class inherits from `~trl.PreTrainedModelWrapper` and wraps a
    `transformers.PreTrainedModel` class. The wrapper class supports classic functions
    ...
    def _init_weights(self, **kwargs):
        ...
        nn.init.eye_(self.v_head.summary.weight)  # 初始化为单位矩阵
        self.v_head.summary.bias.data.zero_()  # 偏置初始化为0
    ...
    def forward(
        ...
        # print("base_model_output1: ", base_model_output)

        if last_hidden_state.device != self.v_head.summary.weight.device:
            last_hidden_state = last_hidden_state.to(self.v_head.summary.weight.device)

        last_hidden_state = self.v_head(last_hidden_state)
        ...
        if lm_logits.dtype != torch.float32:
            lm_logits = lm_logits.float()
        # print("base_model_output: ", base_model_output)
